...
folder = '/Volumes/My Passport/BDD/NetVideos';

cd(folder)

allmovienames = dir('world*');

howmanymovies = length(allmovienames);

for movie = 1:howmanymovies
    
    if movie > 1
        moviename = ['world ' num2str(movie) '.mp4'];
    else
        moviename = 'world.mp4';
    end
    
    movieFullFileName = fullfile(folder, moviename);
    
    message = ['Splitting ' moviename];
    disp(message)
...
%         button = questdlg(promptMessage, 'Recall Movie?', 'Yes', 'No', 'Yes');
        ...
        newmovie = ['New' moviename];
        writerObj = VideoWriter(newmovie);
        open(writerObj);
